...
public class SMSNotificationWorkItemHandler implements WorkItemHandler {
	...
	public void executeWorkItem(WorkItem wi, WorkItemManager wim) {
		this.logger.info("=> SMSNotificationWorkItemHandler in: Process Id::  " + wi.getProcessInstanceId());
		String taskName = "";
		...
		try {
			...
			String initOffer = migrationOutputBean.getResponseMigration().getOffreInitBscs().getOffreDes();
			String targetOffer = migrationOutputBean.getResponseMigration().getOffreTargetBscs().getOffreDes();
			messageDef = reengagementService.getConfig().getPropValues("migrationOK").replace("#offre1#", initOffer)
